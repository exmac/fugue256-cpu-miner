...
static void FUGUEHash(void *state, const void *input,int mode)
{
	...
	sph_fugue256(&ctx,input,80);
	sph_fugue256_close(&ctx,hashA);

    memcpy(state, hashA, 32);	
}

int scanhash_FUGUE(int thr_id, uint32_t *pdata, const uint32_t *ptarget,
...
//	}
	...
	if (ptarget[7]==0) {
		do {
			...
			FUGUEHash(hash64, &endiandata,alg);
